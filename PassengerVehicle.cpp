...
{
	char* cost = new char[5];
	char* numberOfPassengers = new char[3];

	std::cout << "\nMax number of pessengers: ";
	InputValidator::IsNumberInputCorrect(in, numberOfPassengers);
	std::cout << "\nTicket cost: ";
	InputValidator::IsNumberInputCorrect(in, cost);

	vehicle.SetMaxNumberOfPersons(atoi(numberOfPassengers));
	vehicle.SetTicketCost(atoi(cost));
